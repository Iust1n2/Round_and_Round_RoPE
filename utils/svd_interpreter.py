"""Taken from transformer_lens.svd_interpreter.py. Modified `_get_OV_matrix` to be compatible with GQA renaming of W_K and W_V matrices. 
See more in `transformer_lens.components.grouped_query_attention.py` for details.
...
class SVDInterpreter:
    ...
    def get_singular_vectors(
        self,
        vector_type: Union[Literal["QK"], Literal["OV"], Literal["w_in"], Literal["w_out"]],
        layer_index: int,
        num_vectors: int = 10,
        ...
        if vector_type == "QK":
            ...
            matrix = self._get_QK_matrix(layer_index, head_index)
            V = matrix.Vh.T
        elif vector_type == "OV":
    ...
    def _get_QK_matrix(self, layer_index: int, head_index: int) -> FactoredMatrix:
        """Gets the QK matrix for a particular layer and head."""
        ...
        attn_module = self.model.blocks[layer_index].attn
        
        W_Q: torch.Tensor = attn_module.W_Q  # property! auto-expands
        W_K: torch.Tensor = attn_module.W_K  

        W_Q_head, W_K_head = W_Q[head_index, :, :], W_K[head_index, :, :]
        return FactoredMatrix(W_Q_head, W_K_head.transpose(-2, -1))
    ...
    def _get_OV_matrix(self, layer_index: int, head_index: int) -> FactoredMatrix:
        ...
        W_V: torch.Tensor = attn_module.W_V  # property! auto-expands
        W_O: torch.Tensor = attn_module.W_O  

        W_V_head, W_O_head = W_V[head_index, :, :], W_O[head_index, :, :]
        return FactoredMatrix(W_V_head, W_O_head)
